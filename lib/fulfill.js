...
const { issueWelcomeLink } = require('./welcome-token');
...
// Creates the customer's account and returns the link that lets them choose a
// first password.
//
// This used to hand back a Supabase recovery link. Those are spent by whoever
// opens them first, and corporate mail scanners open links to check them — so
// the customer received a dead link. Our own token is spent by submitting a
// password instead, which a scanner never does.
async function createAccessLink(supabase, email, name, purchaseId) {
  ...
  return issueWelcomeLink(supabase, {
    email: email,
    name: name,
...
async function fulfillPurchase({ email, name, product, reference, amount, provider }) {
  ...
  try {
    accessLink = await createAccessLink(supabase, email, name, inserted && inserted.id);
  } catch (linkError) {
